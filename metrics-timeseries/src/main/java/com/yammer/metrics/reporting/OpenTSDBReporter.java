...
public class OpenTSDBReporter extends AbstractTimeSeriesReporter {
     ...
     * @param host   the host name of graphite server (carbon-cache agent)
     * @param port   the port number on which the graphite server is listening
     * @param prefix the string that is prepended to all names reported to tsdb
     * @param tags   the list of key=value tags to associate with each TSD point
     */
    public static void enable(long period, TimeUnit unit, String host, int port, String prefix, List<String> tags) {
        enable(Metrics.defaultRegistry(), period, unit, host, port, prefix, tags);
    }
     ...
     * @param tags            the list of key=value tags to associate with each TSD point
     */
    public static void enable(MetricsRegistry metricsRegistry, long period, TimeUnit unit, String host, int port, String prefix, List<String> tags) {
        enable(metricsRegistry, period, unit, host, port, prefix, tags, MetricPredicate.ALL);
    }
     ...
     */
    public static void enable(MetricsRegistry metricsRegistry, long period, TimeUnit unit, String host, int port, String prefix, List<String> tags, MetricPredicate predicate) {
        try {
     ...
     * @param host   is graphite server
     * @param port   is port on which graphite server is running
     * @param prefix is prepended to all names reported to tsdb
     * @param tags   the list of key=value tags to associate with each TSD point
     *
     * @throws IOException if there is an error connecting to the OpenTSDBReporter server
     */
    public OpenTSDBReporter(String host, int port, String prefix, List<String> tags) throws IOException {
     ...
     * @throws IOException if there is an error connecting to the OpenTSDBReporter server
     */
    public OpenTSDBReporter(MetricsRegistry metricsRegistry, String host, int port, String prefix, List<String> tags) throws IOException {
        this(metricsRegistry, host, port, prefix, tags, MetricPredicate.ALL);
    }

    protected OpenTSDBReporter(MetricsRegistry metricsRegistry, String host, int port, String prefix, List<String> tags, MetricPredicate predicate) {
        super(metricsRegistry, "opentsdb-reporter", host, port, prefix, predicate);
    ...
    protected String formatDoubleField(String sanitizedName, long epoch, double value) {
        return String.format(locale, "put %s%s %d %2.2f %s\n", prefix, sanitizedName, epoch, value, tags);
    }

    @Override
    protected String formatLongField(String sanitizedName, long epoch, long value) {
        return String.format(locale, "put %s%s %d %d %s\n", prefix, sanitizedName, epoch, value, tags);
    }

    @Override
    protected String formatGaugeField(String sanitizedName, long epoch, GaugeMetric<?> gauge) {
        return String.format(locale, "put %s%s %d %s %s\n", prefix, sanitizedName, epoch, gauge.value(), tags);
    }
}
